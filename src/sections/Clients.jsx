    ...
    <div className={`${darkMode ? 'dark bg-black' : 'light bg-transparent'}`}>
      <section id='testimonials' className='lg:w-[95%] w-full h-fit m-auto bg-cover bg-center rounded-xl flex justify-center flex-col items-start lg:px-20 px-6 py-20 gap-y-20'>
        <div className='flex flex-col justify-center items-start gap-4'>
          ...
          <h1 data-aos='zoom-in' className='text-black dark:text-white text-[40px] font-semibold leading-10'>What our clients <br /> are saying about us</h1>
        </div>
        <div id='clients-box' className='grid lg:grid-cols-3 grid-cols-1 justify-center items-center gap-8 w-full'>
          {client.map((item, index) => (
            <div data-aos='zoom-in' data-aos-delay='200' key={index} className='bg-white dark:bg-gray-900 hover:scale-100 cursor-pointer hover:bg-red-300    p-12 flex flex-col justify-center items-center gap-6 rounded-xl w-full'>
              <div className='flex justify-start items-center w-full gap-4'>
                <img src={item.image} alt="" className='w-[70px] h-[70px] transform hover:scale-110 transition-transform duration-300 rounded-full ' />
